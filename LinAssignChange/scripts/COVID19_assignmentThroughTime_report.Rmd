...
The default inputDir should be changed for your folder if not from NML. 
...
linlist<-lapply(pdates,function(pdate){
  datDir<-paste0(inputDir,pdate)
  oplearn=NULL
  
  if(file.exists(paste0(datDir, "/lineage_report.csv"))){
    oplearn<-read.csv(paste0(datDir, "/lineage_report.csv"))
    ...
    warning(paste(paste0(datDir, "/lineage_report.csv"), "isn't available."))}
  oplearn
