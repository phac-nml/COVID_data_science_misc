...
simplifiedPie<-function(x,topl=5,nameAdd=""){
  if(length(x)>0){
    ...
    piegg <- ggplot(piedat, aes(x="", y=Freq, fill=cate)) +
      ...
      geom_text(aes(label = Freq), position = position_stack(vjust=0.5)) + labs(x = NULL, y = NULL, fill = "Lineage Change")+ theme_void()+ ggtitle(paste("Change pairs from", nameAdd))
    print(piegg)
...
Figures labelling top 10 lineages that are significant & those may or may not be significant but with greater than 10% in changes.
The color legend reflects the level of significance in change from binomial tests: -log10(Bonferroni corrected p).
